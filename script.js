...
const CATEGORIES = [
  { name:'Hair Care', icon:'💆', count: PRODUCTS.filter(p=>p.category==='Hair Care').length,
    ...
    alt:'Woman with healthy beautiful hair' },
  { name:'Digestive Health', icon:'🫁', count: PRODUCTS.filter(p=>p.category==='Digestive Health').length,
    photo:'https://images.unsplash.com/photo-1512290923902-8a9f81dc236c?w=400&q=80',
    alt:'Person with healthy digestion' },
  { name:'Immunity', icon:'🛡️', count: PRODUCTS.filter(p=>p.category==='Immunity').length,
    ...
    alt:'Strong healthy person' },
  { name:"Men's Health", icon:'💪', count: PRODUCTS.filter(p=>p.category==="Men's Health").length,
    photo:'https://images.unsplash.com/photo-1590086782957-93c06ef21604?w=400&q=80',
    alt:'Healthy man' },
  ...
  { name:'Infant Care', icon:'👶', count: PRODUCTS.filter(p=>p.category==='Infant Care').length,
    photo:'https://images.unsplash.com/photo-1555252333-9f8e92e65df9?w=400&q=80',
    alt:'Happy baby' },
  { name:'Raw Herbs', icon:'🌾', count: PRODUCTS.filter(p=>p.category==='Raw Herbs').length,
    photo:'https://images.unsplash.com/photo-1515586000433-45406d8e6662?w=400&q=80',
...
function renderCategories() {
  ...
  if (!grid) return;
  grid.className = 'categories-grid-photo reveal-group';
  grid.innerHTML = CATEGORIES.map(c => `
    <div class="category-photo-card" onclick="filterByRemedy('${c.name}')">
      <img src="${c.photo}" alt="${c.alt}"
           onerror="this.style.display='none';this.nextElementSibling.style.background='var(--green-mid)'" />
      <div class="category-photo-overlay">
        <div class="category-photo-name">${c.name}</div>
        <div class="category-photo-count">${c.count} product${c.count !== 1 ? 's' : ''}</div>
      </div>
    </div>
  `).join('');
}
      ...
      <div class="product-info">
        ...
        <div class="product-footer">
          ...
          <button class="btn-add ${inCart ? 'added' : ''}" onclick="addToCart(${p.id}, this)">
            ${inCart ? `<svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20,6 9,17 4,12"/></svg> Added` : 'Add to Cart'}
          </button>
        </div>
      </div>
    </div>
  ...
  if (cart.length === 0) {
    ...
    itemsEl.innerHTML = cart.map(c => `
      <div class="cart-item">
        ...
        <button class="cart-item-remove" onclick="removeFromCart(${c.id})"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3,6 5,6 21,6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg></button>
      </div>
    `).join('');
